...
The textbook's first example involves Hubble data (and thus an explicitly linear model). Using the mpg dataset, I couldn't exactly replicate the necessary random scatter of residuals about zero (i.e. there are some potential trends in the residuals) when modeling highway mpg as a function of engine displacement.

```{r mpg_load, collapse=TRUE}
mpg <- mpg %>% mutate_if(is.character, as.factor)
model_0 <- lm(hwy~displ, data = mpg) # would use xvar-1 to signify no intercept
par(mfcol = c(1,2))
plot(fitted(model_0), xlab = "fitted values", ylab = "residuals")
plot(residuals(model_0), xlab = "fitted values", ylab = "residuals")
```

### T-tests and confidence intervals for hypothesized value of linear coef.

Nonetheless, we can use the example to show how to estimate whether a hypothesized value for the linear coefficient is consistent with the data. Let's say we expect a 1-mpg decrease in the highway rating per additional liter of engine displacement (i.e. coefficient = -10). Then, using a t-test...

```{r ttest}
hypo_val <- -1 # mpg/L
t.stat <- (summary(model_0)$coefficients["displ",][[1]] - hypo_val) / summary(model_0)$coefficients["displ",][[2]]
pt(t.stat,df=nrow(mpg)-1)*2 # to account for probability in either direction of null hypothesis
```
...
```{r}
coef(model_0)[["displ"]]*(1 + qt(c(0.025,0.975), df = nrow(mpg)-1))
```
...
```{echo=FALSE}
GGally::ggpairs(select_if(mpg, is.numeric) %>% select(-year))
```

```{r}
pairs(select_if(mpg, is.numeric) %>% select(-year))
```

However, of all variables in the mpg dataset, we will try adding the class of vehicle as another predictor variable to improve the residuals' distribution. Without getting too unreasonble w.r.t. outlier removal, we also omit some high-mileage vehicles and two-seaters to see if model residuals improve.

```{r,collapse=TRUE}
mpg_slim <- filter(mpg, hwy < 40 & class != "2seater") %>% 
  mutate(class = as.factor(as.character(class))) 
model_1 <- lm(hwy~displ+class, data = mpg_slim)
t.stat2 <- (summary(model_1)$coefficients["displ",][[1]] - hypo_val) / summary(model_1)$coefficients["displ",][[2]]
pt(t.stat2,df=nrow(mpg_slim)-1)*2 # to account for probability in either direction of null hypothesis
par(mfrow = c(2,2))
plot(model_1)
```
...
```{r}
model_null <- lm(hwy~1, mpg_slim)
model_2 <- lm(hwy~class, mpg_slim)
